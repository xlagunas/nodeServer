    ...
    socket.on('login', function(msg, cb){
        ...
        .exec(function(error, loggedUser){
           ...
           if (loggedUser.password === msg.password){
               loggedUser.currentStatus = 'ONLINE';
               console.log("user: "+loggedUser);
               var currentUser = {socket: socket, user: loggedUser};
               ...
               cb({
                   user: currentUser.user.toContact(),
                   contacts: {
                       accepted: currentUser.user.accepted,
                       requested:currentUser.user.requested,
                       pending: currentUser.user.pending,
                       blocked: currentUser.user.blocked
                   }
               });
           }
           else{
               cb({error: "password error"});
           }
